...
const AdminProject = () => {
  ...
  const [previewHero, setPreviewHero] = useState(null); // Added for hero preview

  // --- PART 2: NEW PROJECT STATE (Original Features) ---
  ...
  const [preview, setPreview] = useState(null); // for new project image preview

  // Load existing Hero data for the Projects page
  useEffect(() => {
    const fetchProjectHero = async () => {
      try {
        ...
        if (res.data) {
          ...
          // If the backend returns an existing image path, show it
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Logic for ProjectHero (DROP and Replace)
      ...
      // 2. Logic for New Project Entry (If Title exists)
      ...
      // Reset Project Entry fields
      setNewProject({
        title: "", description: "", techStack: "", githubLink: "",
        liveLink: "", demoLink: "", docsLink: "", image: null,
      });
      setPreview(null);
      setHeroImage(null);
    } catch (err) {
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", padding: "40px 20px", fontFamily: "Segoe UI, sans-serif" }}>
      <div style={{ maxWidth: "700px", margin: "0 auto" }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* --- SECTION 2: CREATE NEW PROJECT (CONTENT) --- */}
          <div style={cardStyle}>
